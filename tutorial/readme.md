...
plot(bassleritree)
```
![plot() tree](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/plot%20tree.png?raw=true)
...
```
![bio1 w/ points](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/bio1%20w%20pts.png?raw=true)
...
```
![treeplot 3.3](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/treeplot.png?raw=true)
...
machu.plotmap(mod)
```
![2 maps](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/2%20models.png?raw=true)
...
## Detailed guide
In this section I'll take you through the process step-by-step again, but with much more detail so that you know what each option does, and also go into the various accessory functions that can aid visualization and accounting for various sources of bias and uncertainty (if you're paranoid). 
### Estimating tip response curves
...
bassleri       232.3243   16.700057  1.0000000       204.40      255.100    1380.014     289.9077   2.2902138        973.80       2058.40
pepperi        245.1000   11.059938  1.0000000       226.45      258.325    1759.836     299.0108   0.8788606       1312.65       2251.70
silverstonei   189.4364    6.815318  0.8878531       177.40      199.000    4569.632     634.5430   0.6909963       3443.60       5477.40
yoshina        242.6364   14.319726  1.0000000       211.75      252.000    1612.290     247.2765 245.7838024       1285.00       2091.75
```
...
legend("topright", legend = c(as.character(unique(occ$species))), col = 4:8, pch=16)
```
![rarefied points](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/rarefied.png?raw=true)
...
machu.respplot(resp)
```
![response curves](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/resp%20curves.png?raw=true)
...
```
![bassleri response curves](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/bassleri%20resp.png?raw=true)
...
```
![bassleri & yoshina curves](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/bassleri%20and%20yoshina%20curves.png?raw=true)
...
```
![bio1 combined](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/bio1%20comb.png?raw=true)
...
```
![all variables combined](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/all%20vars%20comb.png?raw=true)
...
```
![tree w time slices](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/time%20slices.png?raw=true)

The current-climate, furthest to the right, simply cuts through the tips; Mis19, 0.787 Ma old (Pleistocene), recovers each of the four immediate ancestors of the extant taxa (perhaps effectively the same (chrono)species); Mpwp, second from the left (3.205 Ma), and M2, first on the left (3.3 Ma), both Late Pliocene. You'll notice that M2 and Mpwp are very close together, but this was a period of intense climatic change so the two datasets are actually quite different. The two Pliocene time-slices recover two ancestral lineages (the ancestor of the *bassleri* group and the immediate ancestor of *A. silverstonei*). The former "taxon" would be called "Node5-Node6", and the latter "Node5-silverstonei". 
...
```
>Note that here I've done one run for each of the three paleoclimatic datasets. I've also used the version of `resp` that was rarefied, so if you're following exactly, you won't get the same results if you aren't using that one.
...
1     Node7-bassleri            7          1 229.7814
2      Node7-yoshina            7          4 233.6800
3      Node6-pepperi            6          2 240.4903
4 Node5-silverstonei            5          3 192.6377

$bio_1_stdev
                name branch_start branch_end    value
1     Node7-bassleri            7          1 16.15918
2      Node7-yoshina            7          4 13.96155
3      Node6-pepperi            6          2 14.06355
4 Node5-silverstonei            5          3 13.88704
...
```
...
$bio_1_mean
                name branch_start branch_end    value
1        Node5-Node6            5          6 232.5129
2 Node5-silverstonei            5          3 197.6693

$bio_1_stdev
                name branch_start branch_end    value
1        Node5-Node6            5          6 14.63296
2 Node5-silverstonei            5          3 13.98116
...
```
...
4      yoshina  0.000000 NA 234.1138
5        Node5 11.577178 NA 215.0910
6        Node6  2.710415 NA 233.5421
7        Node7  2.267554 NA 232.8640
...
4      yoshina  0.000000 NA 13.51322
5        Node5 11.577178 NA 14.30706
6        Node6  2.710415 NA 14.65221
7        Node7  2.267554 NA 14.80499
...
```
...
```
![visualization of ancestors](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/ancestor%20vis.png?raw=true)
...
1        Node5-Node6            5          6 232.3152 220.9434 228.5246 236.1058 243.6870
2 Node5-silverstonei            5          3 197.8670 190.8334 195.5225 200.2115 204.9006

$bio_1_stdev
                name branch_start branch_end    value     unc1     unc2     unc3     unc4
1        Node5-Node6            5          6 14.62926 12.94527 14.06793 15.19059 16.31325
2 Node5-silverstonei            5          3 13.98486 12.94328 13.63767 14.33206 15.02644
...
```
...
#### Accounting for uncertainty in divergence time estimation
...
```
![3 trees](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/3trees.png?raw=true)
...
```
![3 trees w/ time-slice](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/3trees%20w%20timeslice.png?raw=true)
...
machu.plotmap(mod)
```
![2 models w rarefied occ data](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/2%20mods%20rarefied.png?raw=true)
...
> You can specify different color ramps using the `col` argument in this function and specifying a number (1:6).
...
```
![4 models of extant spp only](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/4%20mods%20extant.png?raw=true)
...
```
![4 models clip Q off bottom](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/4%20mods%20clip%20Q.png?raw=true)

The top two models are from the `mod` object we previously made, where `clip.Q` was set to TRUE. The bottom two models are from `mod.clip.Q`, where `clip.Q` is set to FALSE. You'll notice that it captures quite a bit more area, albeit of lower suitability.
#### Creating binary models
Another option is the `resp.curv` argument, which tells Machuruku that you want to use response curve information to characterize the niche. If you set this argument to FALSE, Machuruku instead sets all values outside of the bounds defined by `lowerQ` and `upperQ` for each climate response curve equal to zero, and all values inside of them to one - effectively creating a uniform distribution defined by `lowerQ` and `upperQ`. In cases where you want a binary suitability surface, this may be useful.
```
# use resp.curv = F
...
```
![4 models resp curv on](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/4%20mods%20resp%20curv.png?raw=true)

Once again, the top two models are from `mod`, with `resp.curv` set to TRUE (default). The bottom two models are with `resp.curv` set to FALSE. As you can see, they capture a similar area, but all of it has the same (maximum) suitability. This is more like saying the taxon is equally likely to inhabit any of these pixels.
...
```
![4 models calc unc on](https://github.com/wxguillo/machuruku/blob/master/tutorial/images/4%20models%20calc%20unc.png?raw=true)

Again, the top two models are from `mod`, with `calc.unc` set to FALSE, and the bottom two models are from `mod.calc.unc`, with `calc.unc` set to TRUE (also notice that we used `ace.M2.unc` as our input, not `ace.M2`). The models incorporating uncertainty are actually a bit narrower in terms of the highly suitable habitat, but should characterize the total variability in suitability better than the other models.
